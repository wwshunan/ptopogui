...
void RFQ::GetField(double time,Particle &particle)
{
  ...
  if(radiu<ERR)
  {
    Er =0.0;
    Et =0.0;
    Ez = -abs(Ul) /2.0 * (A10 *sin(k*z) *Bessin(0, k*radiu) *k + A30 *sin(3*k*z) *Bessin(0,3*k*radiu) * 3 * k ) * pow(-1.0,eleNum+1);
  }
  else
  {
...
//    b03=Bessin(0,3*k*radiu);
    ...
    Er =   abs(Ul)/2.0* (   A01*2*radiu           / pow(r0,2) * cos(2*theta)
                         ...
                         +  A10 * Bessin(1,  k*radiu)  *k *cos(  k*z)               *pow(-1.0,eleNum+1)
                         +  A30 * Bessin(1,3*k*radiu)*3*k *cos(3*k*z)               *pow(-1.0,eleNum+1)
                         +  A12 * Bessin(5,  k*radiu)  *k *cos(  k*z) *cos(4*theta) *pow(-1.0,eleNum+1)
                         +  A32 * Bessin(5,3*k*radiu)*3*k *cos(3*k*z) *cos(4*theta) *pow(-1.0,eleNum+1)
                         +  A21 * Bessin(3,2*k*radiu)*2*k *cos(2*k*z) *cos(2*theta) *pow(-1.0,eleNum+1)
                         +  A32 * Bessin(7,2*k*radiu)*2*k *cos(2*k*z) *cos(6*theta) *pow(-1.0,eleNum+1));
    
    Et =  -abs(Ul)/2.0* (   A01 * pow(radiu/r0,2)                    *sin(2*theta)*2 
                         +  A03 * pow(radiu/r0,6)                    *sin(6*theta)*6 
                         
                         +  A12 * Bessin(4,  k*radiu) *cos(  k*z)    *sin(4*theta)  *4  *pow(-1.0,eleNum+1)
                         +  A32 * Bessin(4,3*k*radiu) *cos(3*k*z)    *sin(4*theta)  *4  *pow(-1.0,eleNum+1)
                         +  A21 * Bessin(2,2*k*radiu) *cos(2*k*z)    *sin(2*theta)  *2  *pow(-1.0,eleNum+1)
                         +  A32 * Bessin(6,2*k*radiu) *cos(2*k*z)    *sin(6*theta)  *6  *pow(-1.0,eleNum+1)
                         ) /radiu;
    ...
    Ez=   -abs(Ul)/2.0* (   A10 * Bessin(0,  k*radiu)*sin(  k*z)  *k
                         ...
                         +  A32 * Bessin(6,2*k*radiu)*sin(2*k*z)*2*k *cos(6* theta))*pow(-1.0,eleNum+1);
  }
...
void RFQ::SetParameter(double posit,const vector<string> & param)
{
  ...
  synPhase     = stod(param.at(5))/180*M_PI;
...
//        getchar();
  ...
  cout<<"RFQ length      : "<<cellPositionEnd.back()<<endl;
...
int RFQ::getEleNum(double z)
{
  ...
  while(count   < cellPosition.size()
      &&z       > cellPositionEnd[count])
  {
    ++count;
  }
